...
const getColorLabel = (color_name, lang = 'en') => {
  if (!color_name) return '';
  if (typeof color_name === 'object') return color_name[lang] || color_name.tr || color_name.en || '';
  return color_name;
};
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/admin/productsList`)
      .then(res => res.json())
      .then(data => {
  ...
  // Benzersiz renkleri al (obje veya string her ikisini de handle et)
          ...
          <Col md={6}>
            ...
            >
              <Image
                ...
                onError={(e) => { e.target.src = "via.placeholder.com/450?text=Image+Not+Found"; }}
              />
            </div>
            <Row className="g-2">
            ...
            {product.hasVariants && (
              <div className="my-4 p-3 bg-light rounded border">
                ...
                <h6 className="fw-bold mb-2">
                  {currentLang === 'tr' ? 'Renk' : 'Color'}:{' '}
                  <span className="text-primary">
                    {getColorLabel(selectedColor, currentLang)}
                  </span>
                </h6>
                <div className="d-flex flex-wrap gap-2 mb-3">
                  {uniqueColors.map((color, idx) => (
                    ...
                    >
                      {getColorLabel(color, currentLang)}
                    </Button>
                  ))}
                </div>
                ...
                {availableSizes.some(v => v.size) && (
                  <>
                    <h6 className="fw-bold mb-2">
                      {currentLang === 'tr' ? 'Beden' : 'Size'}:{' '}
                      <span className="text-primary">{selectedVariant?.size}</span>
                    </h6>
