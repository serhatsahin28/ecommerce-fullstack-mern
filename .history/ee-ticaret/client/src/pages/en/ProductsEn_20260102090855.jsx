  ...
  const filteredProducts = useMemo(() => {
    ...
    let processed = result.map(p => {
      ...
      return {
        ...
        displayPrice: dPrice,
        displayStock: dStock,
        displayImage: dImage?.startsWith('http') ? dImage : `${baseUrl}${dImage}`
      };
    });

    const query = searchQuery.toLowerCase();
    if (query) {
      processed = processed.filter(p => p.name.toLowerCase().includes(query));
    }
    ...
    if (sortBy === 'rating_desc') processed.sort((a, b) => (b.rating || 0) - (a.rating || 0));
