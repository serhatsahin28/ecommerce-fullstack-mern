      ...
      if (token) {
        const fetchUserInfo = async () => {
          try {
            ...
            // *** DÜZELTME: API'den gelen Türkçe anahtar kelimeleri İngilizce state'e maple ***
            setGuestInfo({
              ...addressData,
              firstName: response.data.ad,      // 'ad' -> 'firstName'
              lastName: response.data.soyad,    // 'soyad' -> 'lastName'
              phone: response.data.telefon, // 'telefon' -> 'phone'
              email: response.data.email,
            });

          } catch (error) {
            console.error("Failed to get user information:", error);
            setGuestInfo(addressData); // API hatasında sadece adres bilgisini göster
          }
        };
