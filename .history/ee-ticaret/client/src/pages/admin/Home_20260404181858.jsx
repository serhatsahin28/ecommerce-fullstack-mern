...
const handleImageUpload = (event) => {
  ...
  // ÖNEMLİ: Dosyayı Firebase'e göndermiyoruz! 
  // Sadece tarayıcıda görmek için bir önizleme URL'si oluşturuyoruz.
  const previewUrl = URL.createObjectURL(file);
  
  setImagePreview({
    file: file,           // Gerçek dosya (en son lazım olacak)
    url: previewUrl      // Ekranda göreceğimiz geçici adres
  });
};
    ...
    <Container fluid className="p-2 p-md-3 p-lg-4">
      ...
      <Modal show={showImageModal} onHide={() => setShowImageModal(false)} size="md" className="responsive-modal">
        ...
        <Modal.Footer className="flex-column flex-sm-row gap-2">
          ...
          <Button variant="primary"  disabled={!imagePreview} className="flex-fill"><FaSave /> Seçili Resmi Kullan</Button>
        </Modal.Footer>
      </Modal>
