...
const RegistrationPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Backend message found:', backendMessage);

            if (backendMessage) {
                // Backend'den gelen hata mesajını çevir
                errorMessage = translateError(backendMessage);
                console.log('Translated error:', errorMessage);
            } 
            // Eğer backend mesajı yoksa status koduna göre belirle
            else {
                switch (err.response?.status) {
                    case 409:
                        errorMessage = 'Bu e-mail adresi zaten kayıtlı. Giriş yapmayı deneyin.';
                        break;
                    case 400:
                        errorMessage = 'Girilen bilgilerde hata var. Lütfen kontrol ediniz.';
                        break;
                    case 422:
                        errorMessage = 'Form verileri geçersiz. Lütfen kontrol ediniz.';
                        break;
                    case 500:
