...
const ProfileTR = () => {
  ...
  const handleEditAddress = (address) => {
    setSelectedAddress(address); // Hangi adresi düzenlediğimizi bilelim
    // Formu, düzenlenecek adresin bilgileriyle doldur
    setNewAddress({
    ...
    setShowEditAddressModal(true); // Düzenleme modal'ını göster
  };
  ...
  const adresGuncelle = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const yanit = await axios.put(`http://localhost:5000/address/update/${selectedAddress._id}`, newAddress, {
        headers: { Authorization: `Bearer ${token}` }
      });

      // State'i daha verimli güncelle: tüm listeyi yeniden çekme
      setKullanici(prev => ({
        ...prev,
        adresler: prev.adresler.map(addr => 
          addr._id === selectedAddress._id ? yanit.data.address : addr
        )
      }));
    ...
    const adresSil = async (addressId) => {
      ...
      try {
        ...
        // State'i güncelle: silinen adresi listeden çıkar
        setKullanici(prev => ({
          ...prev,
          adresler: prev.adresler.filter(addr => addr._id !== addressId)
        }));

        setSuccess("Adres başarıyla silindi.");
      } catch (err) {
        console.error("Adres silme hatası:", err.response || err);
                ...
                {kullanici.adresler.length === 0 ? (
                  ...
                  <Row>
                    {kullanici.adresler.map((adres) => (
                      <Col md={6} className="mb-4" key={adres._id}>
                        <Card className="h-100 border-0 shadow-sm">
                          ...
                          <Card.Footer className="d-flex justify-content-end bg-light">
                            ...
                            <Button variant="outline-secondary" size="sm" onClick={() => adresSil(adres._id)}>
