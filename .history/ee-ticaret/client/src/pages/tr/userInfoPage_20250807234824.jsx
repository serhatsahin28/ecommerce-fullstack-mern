...
const UserInfoPage = () => {
  ...
  const saveNewAddress = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/address/add', {
        method: 'POST',
        headers: {
    ...
    <Container className="py-5">
      ...
      {addresses.length === 0 ? (
        <div className="text-center">
          <p className="text-muted mb-3">Henüz kayıtlı adresiniz bulunmuyor.</p>
          <p className="text-muted">Devam etmek için yeni adres ekleyin.</p>
        </div>
      ) : (
        <Button 
          variant="primary" 
