...
const UserInfoPage = () => {
  ...
  const handleSave = async () => {
    try {
      // Kullanıcı ve adres verilerini ayır
      const { ad, soyad, telefon, adres_detay, sehir, ilce, posta_kodu } = userInfo;
      const baseData = { ad, soyad, telefon };

      // Adres dolu mu?
      const adresDolu = adres_detay || sehir || ilce || posta_kodu;

      let bodyToSend = { ...baseData };

      if (adresDolu) {
        const confirmSave = window.confirm("Adres bilginizi kaydetmek ister misiniz?");
        if (confirmSave) {
          bodyToSend.adresler = [
            { adres_detay, sehir, ilce, posta_kodu }
          ];
        }
      }
