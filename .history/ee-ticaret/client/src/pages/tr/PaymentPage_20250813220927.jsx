  ...
  const extractSavedCardToken = (savedCard) => {
    ...
    if (savedCard.cardToken) {
      return {
        ...
        cardUserKey: savedCard.cardUserKey || null
      };
    }
    ...
    const handleSubmit = async (e) => {
      ...
      // Kayıtlı kart seçiliyse CVC kontrolü
      if (selectedSavedCard && !card.cvc) {
        setError('Güvenlik kodu (CVC) gereklidir.');
        setLoading(false);
        return;
      }
      ...
      if (!selectedSavedCard) {
        const requiredFields = ['cardHolderName', 'cardNumber', 'expiry', 'cvc'];
        const missingFields = requiredFields.filter(field => !card[field]?.trim());

        if (missingFields.length > 0) {
          setError('Tüm kart bilgileri doldurulmalıdır.');
          setLoading(false);
          return;
        }
      }
      ...
      const paymentData = {
        ...
        savedCardId: selectedSavedCard ? (selectedSavedCard.id || selectedSavedCard._id) : null,
      ...
      try {
        console.log("Payment Data:", {
          ...paymentData,
          card: paymentData.card ? { ...paymentData.card, cardNumber: '**** **** **** ****', cvc: '***' } : null
        });
