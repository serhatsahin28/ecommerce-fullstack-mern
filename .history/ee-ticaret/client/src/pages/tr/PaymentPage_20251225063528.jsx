...
const PaymentPage = () => {
    ...
    try {
      ...
      const orderData = {
        ...
        payment: {
          method: 'iyzico',
          status: 'success',
          transactionId: paymentResponse.paymentId, // Iyzico'nun verdiği asıl işlem ID'si
          referenceId: paymentResponse.conversationId, // İşlem sırasında gönderilen referans kodu
          date: new Date(),
          savedCardUsed: selectedSavedCard !== null
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    // DÜZELTME: card.cardNumber kontrolü eklendi
    const cardNumber = selectedSavedCard === null
      ? (card.cardNumber || '').replace(/\D/g, '')
      : (selectedSavedCard.kart_numarasi || '').replace(/\s/g, '');

    // DÜZELTME: cardHolderName kontrolü eklendi
    const cardHolderName = selectedSavedCard === null
      ? card.cardHolderName?.trim() || ''
      : selectedSavedCard.kart_ismi || '';
    ...
    const paymentData = {
      ...
      // For new card - DÜZELTME: Güvenli erişim eklendi
    ...
    try {
      ...
      if (paymentResponse.data.success) {
        ...
        console.log();
        setTransactionId(paymentResponse.data.paymentId);
        setReferenceId(paymentResponse.data.conversationId);
