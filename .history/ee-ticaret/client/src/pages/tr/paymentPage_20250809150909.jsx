...
const PaymentPage = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  // Kayıtlı kart için CVC kontrolü
  if (selectedSavedCard && (!card.cvc || card.cvc.length < 3)) {
    setError('CVC code is required for saved card.');
    setLoading(false);
    return;
  }
  ...
  // DÜZELTME: Çift tanımlama hatası düzeltildi
  const paymentData = {
    ...
    // DÜZELTME: Kayıtlı kart bilgileri tek yerde tanımlandı
    savedCardId: selectedSavedCard?._id || null,
    savedCardToken: selectedSavedCard?.token || null,
    cvc: card.cvc, // Her durumda CVC gerekli

    // Yeni kart bilgileri (sadece yeni kart seçiliyse)
    card: selectedSavedCard === null
      ...
      : null // Kayıtlı kart kullanılıyorsa null gönder
  };

  try {
    const token = localStorage.getItem('token');
    
    // DÜZELTME: paymentResponse değişkeni tanımlandı
    const paymentResponse = await axios.post('http://localhost:5000/pay', paymentData, {
      headers: {
        "Content-Type": "application/json",
        ...(paymentData.savedCardId && token ? { Authorization: `Bearer ${token}` } : {})
      }
    });

    // DÜZELTME: Response kontrolü düzeltildi
    ...
    if (err.response?.status === 404) {
      setError('Payment service is not available. Please check server configuration.');
