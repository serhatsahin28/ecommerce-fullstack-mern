...
const ProfileTR = () => {
  ...
  const kartSil = async (paymentToDelete) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Ödeme yöntemi silinemedi.');
    }
  }
          ...
          <Card className="shadow-sm border-0 mb-4">
            <Card.Body>
              ...
              {kullanici.odeme_yontemleri.length === 0 ? (
                ...
                // Bu kısmı Row içine aldım
                <Row>
                  {kullanici.odeme_yontemleri.map((kart, index) => {
                    const kartId = kart.id || kart._id || `kart-${index}`;

                    return (
                      <Col md={6} key={kartId} className="mb-4">
                        <Card className="h-100 border-0 shadow-sm">
                          <Card.Header className="bg-danger text-white d-flex justify-content-between align-items-center">
                            <div>
                              <i className={`fab fa-cc-${(kart.kart_tipi || '').toLowerCase()} fa-2x me-2`}></i>
                              <span className="fw-bold">{kart.kart_ismi || 'İsimsiz Kart'}</span>
                            </div>
                            {kart.varsayilan && <Badge bg="light" text="dark">Varsayılan</Badge>}
                            {!kartId && <Badge bg="warning" text="dark">Geçersiz ID</Badge>}
                          </Card.Header>

                          <Card.Body>
                            <ListGroup variant="flush">
                              <ListGroup.Item>
                                <b>Kart Numarası:</b> {kart.kart_numarasi || 'Bilgi yok'}
                              </ListGroup.Item>
                              <ListGroup.Item>
                                <b>Son Kullanma:</b> {kart.son_kullanma || 'Bilgi yok'}
                              </ListGroup.Item>
                              <ListGroup.Item>
                                <b>ID:</b> <small className="text-muted">{kartId}</small>
                              </ListGroup.Item>
                            </ListGroup>
                          </Card.Body>
                          ...
                          <Card.Footer className="d-flex justify-content-end bg-light">
                            <Button
                              ...
                              size="sm"
                              onClick={() => kartSil(kart)}
                              disabled={!kartId || kartId.includes('geçersiz')}
                            >
