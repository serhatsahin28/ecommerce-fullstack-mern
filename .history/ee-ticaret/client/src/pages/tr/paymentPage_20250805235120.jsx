...
// export default PaymentPage;import React, { useState, useEffect } from 'react';
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedCart) {
      try {
        ...
        // JSON'dan gelenin bir dizi olduğundan emin olalım
        if (Array.isArray(parsedCart)) {
          setCart(parsedCart);
        } else {
          setCart([]); // Eğer dizi değilse, boş dizi ata
        }
      } catch (e) {
        console.error("Sepet verisi okunurken hata oluştu:", e);
        setCart([]); // Hata durumunda boş dizi ata
      }
    }
  }, []);
  ...
  const handleChange = (e) => {
    // ... Orijinal kodunuz, hiç dokunulmadı ...
  };
  
  // === HATA DÜZELTMESİ BURADA ===
  const calculateTotal = () => {
    // Eğer 'cart' bir dizi değilse (null, undefined vb. ise), direkt 0 döndür.
    if (!Array.isArray(cart)) {
      return 0;
    }
    // 'cart' bir diziyse, hesaplamaya devam et.
    return cart.reduce((acc, item) => {
      const price = item.price || item.fiyat || 0;
      const quantity = item.quantity || item.adet || 1;
      return acc + (price * quantity);
      ...
      <Container>
        ...
        <Row>
          <Col lg={7} className="mb-4 mb-lg-0">
            {/* Adres ve Kart Bilgileri Kartları */}
            {deliveryInfo && (
              <Card className="shadow-sm border-0 rounded-3 mb-4">
                ...
                <Card.Body className="p-4 pt-2">
                  <Row className="gy-3">
                    <Col md={12}><div><small className="text-muted">Alıcı</small><p className="fw-semibold mb-0">{deliveryInfo.ad ? `${deliveryInfo.ad} ${deliveryInfo.soyad}` : deliveryInfo.adres_ismi}</p></div></Col>
                    <Col md={12}><div><small className="text-muted">Adres</small><p className="fw-semibold mb-0">{deliveryInfo.adres_detay}</p></div></Col>
                    <Col md={6}><div><small className="text-muted">İlçe / Şehir</small><p className="fw-semibold mb-0">{deliveryInfo.ilce}, {deliveryInfo.sehir}</p></div></Col>
                    <Col md={6}><div><small className="text-muted">Posta Kodu</small><p className="fw-semibold mb-0">{deliveryInfo.posta_kodu}</p></div></Col>
                  </Row>
                </Card.Body>
              </Card>
            )}
            ...
            <Card className="shadow-sm border-0 rounded-3">
              ...
              <Card.Body className="p-4">
                ...
                <Form id="payment-form" onSubmit={handleSubmit}>
                   {/* Form Alanları */}
                   {/* ... Orijinal Form kodunuz olduğu gibi çalışır ... */}
                </Form>
              </Card.Body>
            </Card>
          </Col>
            ...
            <div className="position-sticky" style={{ top: '20px' }}>
                    ...
                    cart.map((item, index) => (
                      <div key={item.id || index} className="d-flex justify-content-between align-items-start mb-3">
                        <div>
                          ...
                          <small className="text-muted">Adet: {item.quantity || item.adet || 1}</small>
                        </div>
                        <p className="fw-bold text-nowrap ms-3">{((item.price || item.fiyat || 0) * (item.quantity || item.adet || 1)).toFixed(2)} TL</p>
                      </div>
                    ))
              ...
              <div className="d-grid mt-4">
                <Button type="submit" form="payment-form" variant="primary" size="lg" disabled={loading || !cart || cart.length === 0}>
                  {loading ? (<><Spinner as="span" animation="border" size="sm" /> <span className="ms-2">Ödeme İşleniyor...</span></>) : `Siparişi Onayla (${totalPrice.toFixed(2)} TL)`}
                </Button>
              </div>
            </div>
          </Col>
