            ...
            <Col md={12} key={order._id}>
              <Card className="shadow-sm border-0">
                <Card.Header className="d-flex justify-content-between align-items-center bg-danger text-white">
                  <h5 className="mb-0">Sipariş #{order.orderCode || order._id.slice(-6)}</h5>
                ...
                <Card.Body>
                  <ListGroup variant="flush">
                    {order.cart && order.cart.length > 0 ? (
                      order.cart.map((item) => (
                        <ListGroup.Item key={item._id.$oid} className="d-flex align-items-center">
                          <img
                            src="/default-product.png" // placeholder resim, kendi projenizde uygun resmi koyun
                            alt={item.name}
                  ...
                  <div className="d-flex justify-content-between">
                    <div>
                      ...
                      {order.shippingInfo?.address}, {order.shippingInfo?.district}, {order.shippingInfo?.city}<br />
                      {order.shippingInfo?.postalCode && <span>Posta Kodu: {order.shippingInfo.postalCode}</span>}
                      {order.shippingInfo?.notes && <p>Notlar: {order.shippingInfo.notes}</p>}
                    </div>
                    ...
                    <div className="text-end">
                      ...
                      <div>
                        <strong>Ödeme Durumu:</strong> {order.payment?.status === 'success' ? <Badge bg="success">Başarılı</Badge> : <Badge bg="danger">Başarısız</Badge>}
                      </div>
                      <div className="mt-2">
