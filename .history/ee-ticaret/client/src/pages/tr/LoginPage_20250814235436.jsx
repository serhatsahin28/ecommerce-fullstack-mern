...
const LoginPageTr = () => {
  ...
  const { fetchCart } = useContext(CartContext); // Cart senkronu

  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false); // success state KALDIRILDI

  // Zaten girişliyse yönlendir
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (token) navigate('/tr');
  }, [navigate]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Sepeti senkronize et (local -> backend merge)
      if (typeof fetchCart === 'function') {
        await fetchCart();
      }

      // Tek toast, 3 sn'de kapanır; toastId çiftlemeyi engeller
    ...
    <Container className="py-5" style={{ maxWidth: '500px' }}>
      <Card className="shadow-sm">
        <Card.Body>
          ...
          {/* ✅ success alert tamamen kaldırıldı */}
