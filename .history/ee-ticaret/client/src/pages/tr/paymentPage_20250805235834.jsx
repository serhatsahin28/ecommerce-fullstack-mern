...
const PaymentPage = () => {
  ...
  useEffect(() => {
    // localStorage'dan verileri al ve logla
    const storedGuestInfo = localStorage.getItem('guestInfo');
    const storedCart = localStorage.getItem('cart');
  ...
  const handleChange = (e) => {
    ...
    // Kart numarası formatını düzenle
    if (name === 'cardNumber') {
      const formattedValue = value.replace(/\D/g, '').replace(/(\d{4})(?=\d)/g, '$1 ').trim();
  ...
  const saveOrderToDatabase = async (paymentResponse) => {
    try {
      const orderData = {
        ...
        cart: cart.map(item => ({
          product_id: item.id || item.product_id,
          name: item.name || item.title || item.urunAdi || 'Ürün',
          category: item.category_title || item.category || 'Genel',
          price: item.price || item.fiyat || 0,
          quantity: item.quantity || item.adet || 1
        })),
  ...
  const handleSubmit = async (e) => {
    ...
    const paymentData = {
      ...
      sepet: cart.map(item => ({
        product_id: item.id || item.product_id,
        translations: item.translations || { tr: { title: item.name || item.title || item.urunAdi || 'Ürün' } },
        category_title: item.category_title || item.category || 'Genel',
        price: item.price || item.fiyat || 0,
        quantity: item.quantity || item.adet || 1
      })),
  ...
  {cart && cart.length > 0 ? (
    <>
      ...
      <ListGroup variant="flush">
        {cart.map((item, index) => {
          // Gerekli verileri güvenli bir şekilde alalım
          const name = item.name || item.title || 'İsimsiz Ürün';
          const quantity = item.quantity || item.adet || 1;
          const price = item.price || item.fiyat || 0;
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="me-3">
                  ...
                  <p className="text-muted small mb-0">
                    {price.toFixed(2)} TL x {quantity} adet
                  </p>
                </div>
                            ...
                            <ListGroup.Item key={index} className="d-flex justify-content-between align-items-center border-0 px-0">
                              <div>
                                <div className="fw-medium">
                                  {item.name || item.title || item.urunAdi || 'Ürün'}
                                </div>
                                <div className="text-muted small mt-1">
                                  {item.category_title || item.category || 'Genel'}
                                </div>
                              </div>
                              <div className="text-end">
                                <div className="text-muted small">
                                  {item.quantity || item.adet || 1} adet × {(item.price || item.fiyat || 0).toFixed(2)} TL
                                </div>
