...
const Header = () => {
  ...
  const { title: categoryTitle, items: categories } = langData.categories;
  ...
  const handleCategoryClick = () => setShowCategories(false);
          ...
          <Nav className="me-auto align-items-center">
            ...
            >
              ...
              <Dropdown.Menu className="category-dropdown w-100 p-3 border-0 shadow-lg mt-0">
                <Container fluid>
                  <h5 className="mb-3 text-danger border-bottom pb-2">{categoryTitle}</h5>
                  <Row className="g-3">
                    {categories.length > 0 ? categories.map((category) => (
                      <Col xs={12} sm={6} md={4} lg={3} xl={2} key={category.id}>
                        <Link
                          to={`/${lng}/${encodeURIComponent(category.title.toLowerCase())}`}
                        ...
                        >
                          <div className="d-flex align-items-center p-2 rounded category-block-hover">
                            ...
                            <div className="category-text">
                              <h6 className="mb-0 fs-6 fw-medium text-dark">{category}</h6>
                              {category.subtitle && (
          ...
          <Nav className="user-nav align-items-center">
            {isAuthenticated() ? (
              <Dropdown align="end">
                <Dropdown.Toggle
                  variant="link"
                  className="text-dark px-2 d-flex align-items-center"
                  id="user-menu"
                >
                  <FaUser size={20} />
                </Dropdown.Toggle>
                ...
                <Dropdown.Menu>
                  <Dropdown.Item as={Link} to={`/${lng}/account`}>
                    {t('profile')}
                  </Dropdown.Item>
                  <Dropdown.Divider />
                  <Dropdown.Item as={Link} to={`/${lng}/ordersPage`} >
                    {t('order')}
            ...
            <Nav.Link
              as={Link}
              to={`/${lng}/cart`}
              className="px-2 position-relative text-dark"
