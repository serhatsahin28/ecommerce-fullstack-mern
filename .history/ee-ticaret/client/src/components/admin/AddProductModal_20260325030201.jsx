...
const AddProductModal = ({ show, onHide, onNotification }) => {
    ...
    const findColorMatch = (value, lang) => {
        const lower = value.toLowerCase();

        if (lang === "tr") {
            return colorDictionary.find(c => c.tr.toLowerCase() === lower);
        } else {
            return colorDictionary.find(c => c.en.toLowerCase() === lower);
        }
    };
    ...
    const handleSave = async () => {
        try {
            ...
            if (variantType !== 'color') {
                ...
                // Renkli: colorGroups → variants dönüşümü (KRİTİK FIX)
                const variants = [];
                ...
                newProduct.colorGroups.forEach((group, gIdx) => {
                    ...
                    // her beden için variant oluştur
                    group.sizes.forEach((sizeItem) => {
                        const trName = group.color_name?.tr || '';
                        const enName = group.color_name?.en || '';
                        const displayName = trName && enName
                            ? `${trName}/${enName}`
                            : trName || enName;

                        variants.push({
                            color_name: group.color_name?.tr || '',
                            color_name_en: group.color_name?.en || '',
                ...
                // 🔥 backend artık bunu bekliyor
                formData.append('variants', JSON.stringify(variants));

                // 🔥 BUNU EKLE (çok önemli)
                                        ...
                                        <Col xs={6}>
                                            ...
                                            <InputGroup size="sm" className="bg-light rounded-3 overflow-hidden border">
                                                <Form.Control
                                                    ...
                                                    value={group.color_name?.tr || ''}
                                                    onChange={e => {
                                                        const val = e.target.value;
                                                        const upd = [...newProduct.colorGroups];

                                                        const match = findColorMatch(val, "tr");

                                                        upd[gIdx].color_name = {
                                                            tr: val,
                                                            en: match ? match.en : upd[gIdx].color_name?.en || ''
                                                        };
