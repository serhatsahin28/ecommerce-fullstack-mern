...
const payWithCard = async (req, res) => {
  try {
    const {
      ...
      sepet,
      totalPrice,
      savedCardId,  // This is expected to be the UCS token from the frontend
      cvc,          // CVC for the saved card
      card          // New card details object
    } = req.body;
    ...
    console.log('🟢 Ödeme isteği geldi:', {
      ...
      savedCardId,
      hasCVC: !!cvc,
      cardLast4: isSavedCard ? 'N/A' : (card && card.cardNumber ? card.cardNumber.slice(-4) : 'N/A')
    });
    ...
    const request = {
      ...
      basketItems: sepet.map((item, index) => {
        ...
        return {
          ...
          // --- FIX STARTS HERE ---
          // Match the data structure from the frontend (translations.tr.name)
    ...
    if (isSavedCard) {
      // Payment with UCS token
      request.paymentCard = {
        ucsToken: savedCardId, // The frontend now sends the correct token here
        cvc: cvc.toString()
      };
    } else {
      ...
      request.paymentCard = {
        ...
        registerCard: '0' // Not saving the new card by default
      };
    }
    ...
    console.error('💥 Kritik Sunucu Hatası:', {
