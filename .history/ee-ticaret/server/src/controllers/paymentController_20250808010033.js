...
const payWithCard = async (req, res) => {
  try {
    ...
    if (isSavedCard) {
      // Kayıtlı kart için UCS token ve CVC kullan
      request.paymentCard = {
        ucsToken: savedCardId,
        cvc: cvc.toString()
      };
    } else {
    ...
    console.log('📤 İyzico isteği:', JSON.stringify({
      ...request,
      paymentCard: {
        ...request.paymentCard,
        cardNumber: isSavedCard ? '****' : request.paymentCard.cardNumber?.slice(0, 6) + '******' + request.paymentCard.cardNumber?.slice(-4),
        cvc: '***'
      }
    }, null, 2));
    ...
    if (result.status !== 'success') {
      ...
      // Özel hata mesajı için düzeltme
      let userMessage = result.errorMessage;
      if (result.errorCode === '5262') {
        userMessage = 'Kayıtlı kartla ödeme hatası. Lütfen farklı bir kart kullanın.';
      }

      return res.status(400).json({
        success: false,
        message: userMessage || 'Ödeme işlemi başarısız',
        errorCode: result.errorCode,
        errorGroup: result.errorGroup
      });
    }
