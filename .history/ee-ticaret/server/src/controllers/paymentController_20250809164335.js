  ...
  try {
    ...
    if (savedCardId) {
      ...
      try {
        ...
        // When registering a card, save both token and user key
        user.odeme_yontemleri.push({
          kart_ismi: cardAlias,
          kart_numarasi: maskedCardNumber,
          son_kullanma: expiry,
          ucsToken: result.cardToken,
          cardUserKey: result.cardUserKey // Add this
        });
        // Kartı bul
        ...
        // Token'ı al
        actualUcsToken = savedCard.ucsToken || savedCard.ucstoken || savedCard.cardToken || savedCard.card_token || savedCard.token;
        isSavedCard = true;
    ...
    // In payWithCard controller, replace the saved card payment section:
    if (isSavedCard && actualUcsToken) {
      console.log('💳 UCS Token ile ödeme yapılıyor:', actualUcsToken.substring(0, 10) + '...');
      request.paymentCard = {
        cardToken: actualUcsToken,
        cardUserKey: user.cardUserKey, // Add this line
        cvc: cvc.toString()
      };
    } else {
      // Yeni kart ile ödeme
