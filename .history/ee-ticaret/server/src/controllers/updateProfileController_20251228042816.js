...
const deleteAddressController = async (req, res) => {
  try {
    ...
    // findByIdAndUpdate kullanarak direkt veritabanı seviyesinde siliyoruz
    const updatedUser = await users.findByIdAndUpdate(
      userId,
      { $set: req.body }, // Sadece gelen verileri güncelle
      { new: true, runValidators: false } // Validasyonu bu işlem için esnet
    );
    ...
    res.status(200).json({
      ...
      adresler: updatedUser.adresler // Güncel listeyi dönmek iyidir
    });
