...
const deleteAddressController = async (req, res) => {
  try {
    ...
    // findByIdAndUpdate kullanarak direkt veritabanı seviyesinde işlem yapıyoruz
    // Bu sayede .save() metodunun tetiklediği genel şema doğrulamasından kaçıyoruz
    const updatedUser = await users.findByIdAndUpdate(
      userId,
      { 
        $pull: { adresler: { _id: addressId } } // Adres dizisinden ilgili ID'yi çıkar
      },
      { 
        new: true, // Güncel kullanıcı verisini geri döndür
        runValidators: false // Silme işleminde diğer alanların doğruluğunu denetleme
      }
    );
    ...
    // Başarılı yanıt dönerken güncel adres listesini de gönderelim ki frontend anında güncellensin
    res.status(200).json({ 
      success: true,
