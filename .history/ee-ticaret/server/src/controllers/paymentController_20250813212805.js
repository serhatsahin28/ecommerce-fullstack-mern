...
const payWithCard = async (req, res) => {
  ...
  try {
    ...
    if (isSavedCard && savedCardData) {
      if (!user.cardUserKey || !savedCardData.cardToken) {
        return res.status(400).json({
          success: false,
          message: 'Bu kayıtlı kart Iyzico’da bulunamadı, lütfen yeni kart bilgisi girin.'
        });
      }

      // Iyzico kayıtlı kart ile ödeme
      request.paymentCard = {
        cardUserKey: user.cardUserKey,
        cardToken: savedCardData.cardToken
      };
    }
    else {
      // Yeni kart ile ödeme
