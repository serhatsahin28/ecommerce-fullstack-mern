...
const uploadHomeImage = async (req, res) => {
  try {
    const file = req.file;
    // Frontend'den gelen eski resmin tam URL'i
    ...
    // --- 1. ADIM: ESKİ RESMİ SİLME İŞLEMİ ---
    if (oldImageUrl && oldImageUrl.includes("firebase")) {
      try {
        // ref(storage, oldImageUrl) dediğinde Firebase URL'den dosya yolunu kendi bulur
        const oldStorageRef = ref(storage, oldImageUrl);
        
        // Dosyayı siler
        await deleteObject(oldStorageRef);
        console.log("Eski dosya başarıyla silindi.");
      } catch (deleteErr) {
        // Eğer dosya zaten yoksa veya isimde bir hata varsa kodun çökmesini engelleriz
        console.error("Silme sırasında hata (Dosya bulunamadı olabilir):", deleteErr.message);
      }
    }

    // --- 2. ADIM: YENİ RESMİ YÜKLEME ---
    // Yeni isim oluşturuyoruz
