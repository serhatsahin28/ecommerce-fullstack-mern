...
const uploadHomeImage = async (req, res) => {
  try {
    const file = req.file;
    const oldImageUrl = req.body.oldImageUrl; // Frontend'den eski resmin linkini göndermelisin
    ...
    if (oldImageUrl && oldImageUrl.includes("firebase")) {
      try {
        // URL'den Firebase'deki dosya yolunu (path) ayıklıyoruz
        // Örn: .../o/home%2F171224... formatındaki yapıyı "home/171224..." yapar
        const fileRef = ref(storage, oldImageUrl); 
        await deleteObject(fileRef);
        console.log("Eski ana sayfa resmi başarıyla silindi.");
      } catch (deleteError) {
        // Eğer dosya zaten silinmişse veya bulunamazsa hata vermemesi için catch içinde tutuyoruz
        console.warn("Eski resim silinemedi (muhtemelen zaten yoktu):", deleteError.message);
      }
    }
    ...
    res.status(500).json({ message: 'İşlem başarısız, resim güncellenemedi.' });
  }
};
