...
const OrdersCancelApprove = async (req, res) => {
  try {
    ...
    // OrdersCancelApprove içindeki 4. adım
    await publishToQueue("order_cancelled_notification", {
      to: order.email, // 'email' yerine 'to' kullanırsan worker ile tam uyumlu olur
      subject: "Sipariş İptal Onayı",
      html: `<h1>Siparişiniz İptal Edildi</h1><p>#${order.orderCode} nolu siparişinizin ${order.totalAmount} tutarındaki iadesi yapılmıştır.</p>`,
      orderCode: order.orderCode
    });
    res.json({
      success: true,
