...
const OrdersCancelApprove = async (req, res) => {
  try {
    const { order_code } = req.body;
    console.log("📥 Gelen Sipariş Kodu:", order_code);

    const order = await Orders.findOne({ orderCode: order_code });

    if (!order) {
      console.log("❌ Sipariş DB'de bulunamadı");
      return res.status(404).json({ message: "Sipariş bulunamadı" });
    }

    console.log("🔍 Sipariş Durumu:", order.orderStatus);

    if (order.orderStatus !== "cancel_pending") {
      console.log("❌ İptal hatası: Sipariş cancel_pending değil!");
      return res.status(400).json({ message: "Sipariş iptal beklemede değil" });
    }

    // Şimdilik burada durdurup sadece kontrol edelim
    return res.status(200).json({ message: "Kontrol başarılı, Iyzico adımına geçilebilir" });
