...
const User = require('../models/User'); // User modelinizi import edin
  ...
  try {
    ...
    if (savedCardId || savedCardToken) {
      ...
      if (!cvc || cvc.length < 3 || cvc.length > 4) {
        ...
        // UCS Token'ı bul
        try {
          // Önce frontend'den gelen token'ı kontrol et
          if (savedCardToken) {
            actualUcsToken = savedCardToken;
            console.log('✅ Frontend\'den UCS token alındı:', actualUcsToken);
          }
          // Eğer token yoksa veritabanından bul
          else if (savedCardId) {
            ...
            if (authHeader) {
              ...
              if (user && user.odeme_yontemleri) {
                const savedCard = user.odeme_yontemleri.find(card => 
                  card._id.toString() === savedCardId || card.id === savedCardId
                );
                
                if (savedCard) {
                  // Olası token alan isimlerini kontrol et
                  ...
                  console.log('🔍 Veritabanından bulunan kart:', {
                    cardId: savedCard._id,
                    cardName: savedCard.kart_ismi,
                    hasToken: !!actualUcsToken,
                    tokenPreview: actualUcsToken ? actualUcsToken.substring(0, 10) + '...' : null
                  });
                } else {
                  console.log('❌ Kayıtlı kart bulunamadı');
                }
              }
            }
          }
          ...
          if (!actualUcsToken) {
            paymentMethodValid = false;
            paymentError = 'Kayıtlı kart token\'ı bulunamadı. Lütfen yeni kart ile ödeme yapın.';
          }
          
        } catch (dbError) {
          console.error('❌ Veritabanı hatası:', dbError);
          paymentMethodValid = false;
          paymentError = 'Kayıtlı kart bilgileri alınamadı. Lütfen yeni kart ile ödeme yapın.';
        }
      }
    }
