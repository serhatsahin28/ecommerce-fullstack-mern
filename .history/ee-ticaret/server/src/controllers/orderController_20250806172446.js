const Order = require('../models/orders'); // Order modelinizi import edin
...
const Order = require('../models/orders'); // Order modelini import ettiğini varsayıyorum
...
const viewOrders = async (req, res) => {
  try {
    ...
    // MongoDB'den kullanıcı siparişlerini getir
    const userOrders = await Order.find({ email: userEmail }).sort({ createdAt: -1 });
    ...
    return res.status(500).json({ success: false, message: 'Sunucuda bir hata oluştu' });
  }
};
