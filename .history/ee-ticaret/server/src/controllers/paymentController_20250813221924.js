...
const payWithCard = async (req, res) => {
  try {
    ...
    let savedCardToken = req.body.savedCardToken;
    ...
    if (isSavedCard) {
      try {
        ...
        console.log('✅ Kayıtlı kart bulundu:', {
          ...
          maskedNumber: savedCardToken.kart_numarasi,
          hasToken: !!savedCardToken.cardToken
        });
    ...
    if (isSavedCard && savedCardToken) {
      ...
      if (savedCardToken.cardToken && savedCardToken.cardUserKey) {
        console.log('✅ Mevcut token kullanılıyor');
        request.cardToken = savedCardToken.cardToken;
        request.cardUserKey = savedCardToken.cardUserKey;
      }
      // Token yoksa kart bilgileriyle ödeme yap
        ...
        request.paymentCard = {
          ...
          cardNumber: savedCardToken.kart_numarasi.replace(/\s/g, ''),
