...
const basketItemShow = async (req, res) => {
    try {
        const userId = req.user.id; // authMiddleware ile user bilgisi gelir

        const basket = await Basket.findOne({ userId: userId }).populate('products.product');
        console.log("basket",basket);
        // products.product → Product modeline ref varsa populate ile detaylı bilgi

        if (!basket) return res.status(200).json({ items: [] });

        // API response: [{id, name, price, quantity}, ...]
        const items = basket.products.map((p) => ({
            id: p.product._id,
            name: p.product.name,
