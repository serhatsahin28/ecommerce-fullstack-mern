...
const addPaymentMethod = async (req, res) => {
  try {
    ...
    const newPaymentMethod = {
      ...
      id: new mongoose.Types.ObjectId().toString() // MANUEL ID ATAMA
    };
...
const deletePaymentMethod = async (req, res) => {
  try {
    ...
    user.odeme_yontemleri = user.odeme_yontemleri.filter(payment => {
      ...
      const pid = payment.id || payment._id || (payment._id ? payment._id.toString() : null);
      return String(payment.id || payment._id) !== String(paymentId);
