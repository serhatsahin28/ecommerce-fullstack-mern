...
const createOrder = async (req, res) => {
  try {
    ...
    // console.log(cart);
    const paymentResult = await iyzico.payment.create(paymentData);
    ...
    const orderData = {
      ...
      payment: {
        ...
        // iyzico paymentId
        iyzicoReference: paymentResult.paymentId,

        // 🔥 REFUND İÇİN ŞART
        transactions: paymentResult.itemTransactions.map(t => ({
          paymentTransactionId: t.paymentTransactionId,
          price: Number(t.price),
          paidPrice: Number(t.paidPrice)
        })),
    ...
    try {
      for (const item of cart) {
        ...
        const updatedHome = await Home.findOneAndUpdate(
          ...
          {
            $inc: {
              "categories.$[].products.$[p].stock": -item.quantity
            }
          },
          {
            arrayFilters: [
      ...
      // ❗ stok hatası olursa siparişi geri al
    ...
    if (userId) {
      console.log("userId var:" + userId + "sepet içerisinde stok eksilmesi gerekiyor");
