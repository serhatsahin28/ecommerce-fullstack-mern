...
const payWithCard = async (req, res) => {
  ...
  try {
    ...
    if (isSavedCard && savedCardData) {
      ...
      const cardToken = savedCardData.kart_token;

      if (!cardUserKey || !cardToken) {
        return res.status(400).json({
          success: false,
          message: 'Kayıtlı kart bilgileri eksik, lütfen yeni kart bilgisi girin.'
        });
      }
      ...
      request.paymentCard = {
        cardHolderName: kartIsmi,
        cardNumber: kartNumarasi.replace(/\s/g, ''),
        expireMonth: sonKullanmaAy,
        expireYear: sonKullanmaYil,
        cvc: cvc,
        registerCard: "1" // Iyzico'da kaydedilsin mi
      };
    }
