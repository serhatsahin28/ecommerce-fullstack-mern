...
const sendOrderLink = async (req, res) => {
    try {
        ...
        const existingOrders = await Order.find({ email: email.toLowerCase().trim() });

        // Eğer sipariş yoksa, link göndermeye gerek yok (Güvenlik için)
        if (!existingOrders || existingOrders.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Bu e-posta adresiyle eşleşen bir sipariş bulunamadı.'
            });
        }
