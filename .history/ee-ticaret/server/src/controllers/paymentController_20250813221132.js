...
const payWithCard = async (req, res) => {
  try {
    ...
    if (isSavedCard) {
      try {
        ...
        console.log('✅ Kayıtlı kart bulundu:', {
          cardId: savedCardData._id,
          maskedNumber: savedCardData.kart_numarasi,
          hasToken: !!savedCardData.cardToken
        });
    ...
    if (isSavedCard && savedCardData) {
      ...
      if (savedCardData.cardToken && savedCardData.cardUserKey) {
      ...
      // Token yoksa kart bilgileriyle ödeme yap
        ...
        if (!savedCardData.kart_numarasi || !savedCardData.son_kullanma || !savedCardData.kart_ismi) {
          return res.status(400).json({
            success: false,
    ...
    if (result.cardToken && result.cardUserKey && isSavedCard && savedCardData) {
      try {
