...
const payWithCard = async (req, res) => {
  try {
    ...
    let user = null; // Kullanıcıyı daha geniş scopeta tanımladık
    ...
    if (savedCardId) {
      ...
      isSavedCard = true;
      console.log(ay);
      const [ay, yıl] = savedCard.son_kullanma.split('/');
      request.paymentCard = {
        cardNumber: savedCard.kart_numarasi,
        expireMonth: ay,
        expireYear: yıl,
        cvc: savedCard.cvc.toString(),
        cardHolderName: savedCard.kart_ismi,
        registerCard: "0"
      };
    }
    else {
