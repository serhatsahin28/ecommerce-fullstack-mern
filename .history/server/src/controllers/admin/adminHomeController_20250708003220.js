...
// Resim yükleme ve veritabanını güncelleme işlemi
const uploadHomeImage = async (req, res) => {
  try {
    const homePageId = req.params.id; // Güncellenecek dökümanın ID'si
    const file = req.file; // Multer tarafından yüklenen dosya bilgisi
    
    // 1. Dosya yüklenmemişse hata döndür
    if (!file) {
      return res.status(400).json({ message: 'Lütfen bir resim dosyası seçin.' });
    }
    
    // 2. Veritabanında saklanacak resim yolunu oluştur
    // Bu yol, frontend'in resme erişebileceği public bir yoldur.
    const imagePath = `/uploads/home/${file.filename}`;
    
    // 3. İlgili 'home' dökümanını bul ve resim alanını güncelle
    // Schema'nızdaki resim alanının adının 'image' olduğunu varsayıyoruz.
    // Eğer farklı bir isimde ise (örn: 'mainImage', 'imageUrl'), 'image' kısmını onunla değiştirin.
    const updatedHome = await home.findByIdAndUpdate(
      homePageId, 
      { image: imagePath }, // Güncellenecek alan ve yeni değeri
      { new: true } // Bu seçenek, güncellenmiş dökümanı geri döndürür
    );
    
    // 4. Güncelleme sonrası döküman bulunamadıysa hata döndür
    if (!updatedHome) {
      // Bu durum, geçersiz bir ID gönderildiğinde oluşabilir.
      return res.status(404).json({ message: 'Güncellenecek anasayfa verisi bulunamadı.' });
    }
    
    console.log("Veritabanı güncellendi, yeni resim yolu:", updatedHome.image);

    // 5. Başarılı yanıtı, yeni resim yolunu da içerecek şekilde gönder
    res.status(200).json({ 
      message: 'Resim başarıyla yüklendi ve kaydedildi!',
      imagePath: imagePath // Frontend'in anında önizleme yapabilmesi için yolu gönderiyoruz
    });
