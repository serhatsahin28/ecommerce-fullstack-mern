...
const uploadHomeImage = async (req, res) => {
  try {
    const file = req.file;
    const homePageId = req.params.id; // Ana dökümanın ID'si
    const { sliderId } = req.body; // FormData'dan gelen sliderId
    ...
    if (!sliderId || !mongoose.Types.ObjectId.isValid(sliderId)) {
        return res.status(400).json({ message: 'Geçersiz veya eksik slayt ID.' });
    }

    // 2. Resim yolunu oluşturma
    const imagePath = `/images/${file.filename}`; // upload.js'e göre dosya yolu

    // 3. MongoDB'de ID Tabanlı Güncelleme
    const updateResult = await home.updateOne(
      { 
        _id: homePageId, 
        'heroSlides.slider_id': new mongoose.Types.ObjectId(sliderId)
      },
      {
        $set: { 'heroSlides.$.image': imagePath }
      }
    );

    if (updateResult.modifiedCount === 0) {
      return res.status(404).json({ message: 'Slayt bulunamadı veya resim zaten aynıydı.' });
    }
    ...
    res.status(200).json({
      ...
      imagePath: imagePath,
      sliderId: sliderId // Frontend'in hangi slaytı güncelleyeceğini bilmesi için
    });
