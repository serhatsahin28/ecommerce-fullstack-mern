...
const saveHomePageData = async (req, res) => {
  try {
    ...
    // Hero slides için slider_id'leri kontrol et ve eksik olanları oluştur
    if (updatedData.heroSlides && Array.isArray(updatedData.heroSlides)) {
      updatedData.heroSlides = updatedData.heroSlides.map(slide => {
        if (!slide.slider_id) {
          const { v4: uuidv4 } = require('uuid');
          slide.slider_id = uuidv4();
        }
        return slide;
      });
    }
    ...
    const updatedDoc = await home.findByIdAndUpdate(homePageId, updatedData, { new: true });
...
// uploadHomeImage fonksiyonunu güncelle
...
//     console.log("Upload request:", { sliderId, homePageId, filename: file?.filename });
...
//     if (!sliderId) {
//       return res.status(400).json({ message: 'Slider ID gerekli.' });
//     }
...
//     // slider_id ile heroSlides içinde doğru slaytı bul
//     const targetSlideIndex = homeDoc.heroSlides.findIndex(slide => 
//       slide.slider_id && slide.slider_id.toString() === sliderId
//     );

//     if (targetSlideIndex === -1) {
//       return res.status(404).json({ message: 'Slayt bulunamadı. Önce slider\'ı kaydedin.' });
//     }

//     // Resim yolunu güncelle
//     homeDoc.heroSlides[targetSlideIndex].image = imagePath;
...
//   }
// };
// adminHomeController.js içindeki ESKİ uploadHomeImage fonksiyonunu SİLİP,
// yerine AŞAĞIDAKİ YENİ ve KISA fonksiyonu yapıştırın:
...
module.exports = { getHomeList, updateHomeList, saveHomePageData, uploadHomeImage, deleteItem,addHeroSlider }
