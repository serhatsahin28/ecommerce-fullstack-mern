...
// Yeni hero slide eklemek için
const addHeroSlide = async (req, res) => {
  try {
    const homePageId = req.params.id;
    const newSlideData = req.body; // örn: { title: "...", description: "...", image: "", slider_id: new ObjectId() } gibi

    if (!homePageId || !mongoose.Types.ObjectId.isValid(homePageId)) {
      return res.status(400).json({ message: 'Geçersiz sayfa ID.' });
    }

    // Zorunlu alanları burada kontrol edebilirsin, örn slider_id yoksa oluştur
    if (!newSlideData.slider_id) {
      newSlideData.slider_id = new mongoose.Types.ObjectId();
    }
    ...
    homeDoc.heroSlides.push(newSlideData);
    await homeDoc.save();

    // Eklenen slide'ı döndür
    const addedSlide = homeDoc.heroSlides[homeDoc.heroSlides.length - 1];
    res.status(200).json({ message: 'Yeni slayt eklendi', slide: addedSlide });
  } catch (err) {
    console.error('Yeni slide ekleme hatası:', err);
    res.status(500).json({ message: 'Sunucu hatası, slide eklenemedi.' });
  }
};


module.exports = { getHomeList, updateHomeList, saveHomePageData, uploadHomeImage, deleteItem,addHeroSlide }
