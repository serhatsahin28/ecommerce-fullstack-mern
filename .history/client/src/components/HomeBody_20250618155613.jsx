...
export default function HomeBody() {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/home')
      ...
      .catch(err => {
        console.error("Home verisi alınamadı:", err);
        setIsLoading(false);
      });
  }, []);
  ...
  // Hero Slides, Banner, Advantages, Stats, Categories
  const { heroSlides = [], banner = {}, advantages = [], stats = [], categories = [] } = homeData;

  // Tüm ürünleri kategori bazında grupla
  const productsByCategory = useMemo(() => {
    const grouped = {};
    categories.forEach(cat => {
      grouped[cat.category_key] = cat.products || [];
    });
    return grouped;
  }, [categories]);

  // Öne çıkan ürünler: burada örnek olarak ilk 4 ürün alınıyor (daha gelişmiş filtreleme için backend veya ek frontend mantığı lazım)
  const featured = useMemo(() => {
    return categories.flatMap(cat => cat.products).slice(0, 4);
  }, [categories]);

  // En çok satanlar (rating’e göre ilk 4)
    ...
    <div className="bg-home">
          ...
          <Carousel fade indicators={false} className="mb-0">
            {heroSlides.map((slide, index) => (
              <Carousel.Item key={index} interval={5000}>
                ...
                >
                  <div
                    className="hero-content rounded-4 shadow-lg p-4 px-5"
      ...
      <Container className="py-5">
        <header className="text-center mb-5">
          <h1 className="display-5 fw-bold text-danger">{homeData.page_title}</h1>
          <p className="lead text-muted">{homeData.page_subtitle}</p>
        </header>

        {/* Öne Çıkan Ürünler */}
        {featured.length > 0 && (
          <section className="mb-5">
            <h2 className="h4 mb-4 section-title">
