...
const Header = () => {
  ...
  const [categoryData, setCategoryData] = useState({ title: '', items: [] });

  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch(`/api/ecommerce-${lng}`);
        const data = await res.json();
        if (data?.categories) setCategoryData(data.categories);
      } catch (err) {
        ...
        <Navbar.Collapse id="main-nav">
          <Nav className="me-auto align-items-center">
            ...
            >
              ...
              >
                {categoryData?.title || 'Kategoriler'}
              </Dropdown.Toggle>

              <Dropdown.Menu className="category-dropdown w-100 p-3 border-0 shadow-lg mt-0">
                <Container fluid>
                  <h5 className="mb-3 text-danger border-bottom pb-2">{categoryData?.title || 'Kategoriler'}</h5>
                  <Row className="g-3">
                    {categoryData.items.length > 0 ? categoryData.items.map((category) => (
                      ...
                      <Col><p className="text-muted">Kategori bulunamadı.</p></Col>
                    )}
                  </Row>
                </Container>
              </Dropdown.Menu>
