...
const AdminHome = () => {
 ...
 const openImageModal = (type, index = null) => {
  ...
  // saveImage fonksiyonu güncellendi
  const saveImage = async () => {
  if (!selectedFile) return;

  const { type, index, sliderId, homePageId } = currentImageField;

  const formData = new FormData();
  formData.append('image', selectedFile);
  formData.append('sliderId', sliderId); // sliderId'yi gönderiyoruz

  try {
    const res = await axios.post(`/api/admin/home/${homePageId}/upload`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });

    const newImagePath = res.data.imagePath;

    // Görseli local state'te güncelle
    if (type === 'heroSlides') {
      handleArrayChange('heroSlides', index, 'image', newImagePath);
    }
    ...
    // Backend’e kaydettikten sonra diğer değişiklikleri de kaydet
    handleSave();
  } catch (error) {
