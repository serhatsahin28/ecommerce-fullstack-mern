    ...
    const handleImageUpload = async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert('Resim yükleme hatası: ' + result.message);
            }
        } catch (err) {
    ...
    setHomePageData(prev => {
        const updatedCategories = prev.categories.map(cat => {
            if (cat.category_key === categoryKey) {
                // Ürünler dizisinin varlığını ve geçerliliğini kontrol et
                const currentProducts = Array.isArray(cat.products) ? cat.products : [];
                console.log('Mevcut ürünler:', currentProducts); // Debug için

                // productId'leri string olarak karşılaştır
                const filteredProducts = currentProducts.filter(p => {
                    const isMatch = String(p.product_id) !== String(productId);
                    console.log('Karşılaştırma:', p.product_id, '!==', productId, '=', isMatch); // Debug için
                    return isMatch;
                });
                ...
                return {
                    ...cat,
                    products: filteredProducts // products4275 yerine products kullanıldı
                };
            }
            return cat;
        });
        ...
        <Container fluid className="p-3 p-md-4">
            ...
            <Modal show={showImageModal} onHide={() => setShowImageModal(false)} size="md">
                ...
                <Modal.Body>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label>Veya URL Girin</Form.Label>
                        <Form.Control 
                            type="url" 
