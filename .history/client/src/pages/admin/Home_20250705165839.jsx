...
const AdminHome = () => {
    ...
    // ==========================================================
    // ===== YENİ VE DOĞRU addProductToCategory FONKSİYONU =====
    // ==========================================================
    const addProductToCategory = (product) => {
        // Backend'deki 'Home' şemasının beklediği yapıya uygun yeni bir obje oluşturuyoruz.
        const productForHomePage = {
            product_id: product._id.toString(), // Ana ürünün `_id`'sini, beklenen `product_id` alanına atıyoruz.
            stock: product.stock,
            price: product.price,
        ...
        setHomePageData(prev => {
            const newCategories = prev.categories.map(cat => {
                if (cat.category_key === currentCategoryKey) {
                    // Ürün zaten var mı diye kontrol ederken, eklenen ürünün kimliğine (_id) bakıyoruz.
                    if ((cat.products || []).some(p => p.product_id === product._id)) {
                        alert("Bu ürün zaten kategoride mevcut.");
                        return cat;
                    }
                    // STATE GÜNCELLEME: Dönüştürdüğümüz, yeni ve doğru objeyi (`productForHomePage`) state'e ekliyoruz.
    ...
    const getAvailableProducts = () => {
        ...
        // Mevcut eklenmiş ürünlerin ID'lerini bir sete topluyoruz. Kontrolü 'product_id' üzerinden yapıyoruz.
        const currentProductIds = new Set((currentCategory.products || []).map(p => p.product_id));

        return allProducts
            .filter(p => p.category_key === currentCategoryKey)
            // Henüz eklenmemiş olanlar (allProducts'taki `_id` ile `currentProductIds`'daki `product_id` eşleşiyor mu diye bakıyoruz)
                                ...
                                <div key={cat.category_key} className="p-3 border rounded mb-3">
                                    ...
                                    <Row>
                                        {(cat.products || []).map(product => (
                                            <Col key={product.product_id} xs={6} md={4} lg={3} className="mb-2">
