...
const AdminHome = () => {
 ...
 // ESKİ handleImageUpload FONKSİYONUNUN YERİNE BU YAZILACAK:
  ...
  const handleImageUpload = async (event) => {
    ...
    // Resim seçilir seçilmez yükleme işlemi için formData hazırlıyoruz.
    const formData = new FormData();
    formData.append('image', file);

    try {
      // Yeni, basitleştirilmiş backend rotamıza istek atıyoruz.
      ...
      if (response.ok) {
        // Gelen resim yolunu önizleme ve kaydetme için state'e alıyoruz.
        setImagePreview(result.imagePath);

        setStatusMessage({
            show: true,
            message: 'Resim yüklendi. Kalıcı olması için modal içerisindeki Kaydet butonuna basın.',
            type: 'info'
        });
