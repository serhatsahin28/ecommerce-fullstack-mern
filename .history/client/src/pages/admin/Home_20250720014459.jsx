...
const AdminHome = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    const isValidUUID = (id) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(id.toLowerCase());

    if (!sliderId || (!mongoose.Types.ObjectId.isValid(sliderId) && !isValidUUID(sliderId))) {
      return res.status(400).json({ message: 'Geçersiz slider ID.' });
    }
    // Burada formData'yı kullanmadan önce tanımlamalısın
    const formData = new FormData();
    formData.append('image', file);
    if (sliderId) {
      console.log('Gönderilen sliderId:', sliderId);
      formData.append('sliderId', sliderId);
    } else {
      console.warn('SliderId yok, upload yapılamıyor.');
    }
