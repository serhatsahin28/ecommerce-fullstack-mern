...
export default function ProductDetailTR() {
  ...
  const isLangMismatch = lang !== 'tr';

  useEffect(() => {
    if (!expectedCategoryKey || isLangMismatch) {
      setNotFound(true);
      return;
    }
  ...
  }, [id, expectedCategoryKey, isLangMismatch]);

  if (notFound) return <Navigate to="/tr/404" replace />;
  if (!product) return <Navigate to="/tr/404" replace />;

  const tr = product.translations?.tr || {};

  return (
    <Container className="py-5">
      {/* Görsel + Bilgi alanları (önceki hal aynı kalıyor) */}
      <Row>
        <Col md={6}>
          <div
          ...
          <Row className="g-2">
            {product.images?.map((img, i) => (
              <Col xs={3} key={i}>
                ...
                >
                  <Image src={img} style={{ height: '100%', width: '100%', objectFit: 'cover' }} fluid />
                </div>
              </Col>
            ))}
          </Row>
        ...
        <Col md={6}>
          ...
          <ul className="mt-3">
            {tr.features?.map((feat, i) => <li key={i}>{feat}</li>)}
          </ul>
