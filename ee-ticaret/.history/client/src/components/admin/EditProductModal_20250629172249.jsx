...
<Form.Group className="mb-3" controlId="editProductImages">
  ...
  {/* Mevcut resimler (URL’den gelenler) */}
  <div className="d-flex flex-wrap mb-2">
    {editProduct.images?.map((img, idx) => (
      <div key={`existing-${idx}`} style={{ position: 'relative', marginRight: 10, marginBottom: 10 }}>
    ...
    {/* Yeni seçilen resimlerin preview’i */}
    {editProduct.newImageFiles?.map((file, idx) => (
      <div key={`new-${idx}`} style={{ position: 'relative', marginRight: 10, marginBottom: 10 }}>
...
<Form.Group className="mb-3" controlId="editProductImages">
  ...
  <Form.Control
    ...
    onChange={e => {
      const files = Array.from(e.target.files);
      // Yeni seçilen resimleri dataURL olarak ekleyelim (preview için)
      files.forEach(file => {
        const reader = new FileReader();
        reader.onloadend = () => {
          setEditProduct(prev => ({
            ...
            newImageFiles: prev.newImageFiles ? [...prev.newImageFiles, file] : [file]
          }));
        };
        reader.readAsDataURL(file);
      });
      // Inputu temizle, böylece aynı dosyayı tekrar seçmek mümkün olur
      e.target.value = null;
    }}
  />
</Form.Group> 
