...
import {
  Container, Row, Col, Form, Button, InputGroup, Spinner, Alert, Card, Toast, ToastContainer
} from 'react-bootstrap';
import {
  FaSearch, FaSortAmountDown, FaSortAmountUp, FaListUl,
...
export default function ProductsEn() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const translatedProducts = Array.isArray(items)
        ? items.map((item) => {
      ...
      setAllProducts(translatedProducts);
      setIsLoading(false);
    }, 200);
  }, [t, currentLang]);
                ...
                <Col key={product.id}>
                  <Card className="h-100 shadow-sm d-flex flex-column">
                    ...
                    <Card.Body className="d-flex flex-column">
                      <Card.Title>
                        <Link to={`/en/products/${product.id}`} className="text-decoration-none text-dark">
                          {product.name}
                        </Link>
                      </Card.Title>
