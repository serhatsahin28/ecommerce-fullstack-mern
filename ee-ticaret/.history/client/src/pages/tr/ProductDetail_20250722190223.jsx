...
import { CartContext } from '../context/CartContext';  // CartContext'i ekle
...
export default function ProductDetailTr() {
  ...
  const { addToCart } = useContext(CartContext);  // Sepete eklemek için context
  ...
  return (
    <>
      {/* Navbar altı toast bildirimi */}
      <div style={{
        position: 'fixed',
        top: '56px', // Bootstrap default navbar yüksekliği
        right: '20px',
        zIndex: 1060,
        minWidth: '250px',
      }}>
        ...
        >
          ...
          <Toast.Body className="text-white">Ürün sepete eklendi!</Toast.Body>
        </Toast>
      </div>
        ...
        {trData.reviews?.length > 0 && (
          <section className="mt-5">
            ...
            {trData.reviews.map((r, i) => (
              <blockquote
                key={i}
                className="blockquote border-start border-4 ps-3 mb-3"
              >
                “{r}”
              </blockquote>
            ))}
