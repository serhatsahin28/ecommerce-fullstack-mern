...
const addNewHeroSlide = async (slideData) => {
  try {
    const response = await fetch(`http://localhost:5000/admin/home/${homePageData._id}/heroSlide/add`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (response.ok) {
      // Yeni eklenen slide'ı gerçek slider_id ile state'e ekle
      const newSlide = result.slide;
      setHomePageData(prev => ({
        ...prev,
        heroSlides: [...prev.heroSlides, newSlide]
      }));
    } else {
      setStatusMessage({ show: true, message: result.message, type: 'danger' });
    }
  } catch (err) {
    setStatusMessage({ show: true, message: 'Sunucu hatası: ' + err.message, type: 'danger' });
  }
};
