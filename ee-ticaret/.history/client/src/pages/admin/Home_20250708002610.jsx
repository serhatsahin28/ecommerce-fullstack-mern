...
const AdminHome = () => {
  ...
  const handleSave = async () => {
    ...
    console.log("homePageData._id", homePageData._id);
    setIsSaving(true);
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatusMessage({ 
          show: true, 
          message: 'Resim başarıyla yüklendi! Şimdi "Resmi Kaydet" butonuna basın.', 
          type: 'success' 
        });
      } else {
        setStatusMessage({ 
  ...
  const saveImage = async () => {
    ...
    if (type === 'heroSlides') {
      ...
      setHomePageData(prev => {
        const newHeroSlides = [...prev.heroSlides];
        newHeroSlides[index] = {
          ...newHeroSlides[index],
          image: imagePreview
        };
  ...
  const removeProductFromCategory = (categoryKey, productId) => {
    ...
    console.log('Silinecek ürün:', { categoryKey, productId });

    setHomePageData(prev => {
      const updatedCategories = prev.categories.map(cat => {
        if (cat.category_key === categoryKey) {
          const currentProducts = cat.products || [];
          console.log('Mevcut ürünler:', currentProducts);

          const filteredProducts = currentProducts.filter(p => {
            console.log('Karşılaştırma:', p.product_id, '!==', productId);
            return p.product_id !== productId;
          });

          console.log('Filtreli ürünler:', filteredProducts);
      ...
      console.log('Güncellenmiş kategoriler:', updatedCategories);
  ...
  return (
    <Container fluid className="p-3 p-md-4">
                  ...
                  <Row>
                    <Col md={10}>
                      ...
                      <InputGroup className="mb-2">
                        ...
                        <Form.Control size="sm" placeholder="/uploads/home/ornek.jpg" value={slide.image || ''} onChange={(e) => handleArrayChange('heroSlides', index, 'image', e.target.value)} />
                    ...
                    <Col md={2} className="d-flex align-items-center justify-content-center flex-column">
                      {slide.image && (
                        <img src={slide.image.startsWith('http') ? slide.image : `http://localhost:5000${slide.image}`} alt="Slide" className="img-thumbnail mb-2" style={{ width: '60px', height: '60px', objectFit: 'cover' }} />
                      )}
      ...
      <Modal show={showImageModal} onHide={() => setShowImageModal(false)} size="md">
        <Modal.Header closeButton>
          <Modal.Title>Resim Seç ve Yükle</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group className="mb-3">
          ...
          {imagePreview && (
            <div className="text-center mb-3">
              ...
              <p className="mt-2 text-success">
                <FaUpload /> Resim yüklendi! Kaydetmek için "Resmi Kaydet" butonuna basın.
              </p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={saveImage} disabled={!imagePreview || isUploading}>
            <FaSave /> Resmi Kaydet
          </Button>
        </Modal.Footer>
      </Modal>
