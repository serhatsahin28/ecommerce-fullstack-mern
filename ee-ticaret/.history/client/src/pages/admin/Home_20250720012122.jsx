...
const AdminHome = () => {
  ...
  const handleSave = async () => {
    ...
    const cleanedData = {
      ...
      heroSlides: homePageData.heroSlides.map(slide => {
        ...
        if (image && typeof image === 'string' && (image.startsWith('/uploads/') || image.startsWith('/images/'))) {
          return { ...rest, image };
        }
        return rest;
      }),
    ...
    try {
      ...
      // Başarılıysa response’tan güncellenmiş veriyi alabilirsin
      return true;
    } catch (err) {
  ...
  const handleImageUpload = async (event) => {
    ...
    if (type === 'heroSlides' && index !== null) {
      ...
      if (!sliderId) {
        // Sadece önizleme oluştur, sunucuya gönderme
    ...
    try {
      ...
      if (response.ok) {
        ...
        await saveImage(); // imagePreview kaydedilecek
      } else {
        setStatusMessage({
          show: true,
  ...
  // const handleImageChangeAndSave = async (e) => {
  //   const file = e.target.files[0];
  //   if (!file) return;

  //   setImagePreview(URL.createObjectURL(file)); // Önizleme için

  //   // 1. İlk önce veriyi kaydet (resim olmadan)
  //   await handleSave();

  //   // 2. Sonra resmi upload et
  //   await handleImageUpload(file);
  // };


  const handleImageChangeAndSave = async (e, index) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    try {
      // 1. İlk önce veriyi kaydet
      await handleSave();

      // 2. En güncel veriyi çek ki sliderId'leri de olsun
      const res = await fetch(`http://localhost:5000/admin/home/${homePageData._id}`);
      const data = await res.json();

      // 3. Güncel state'e al
      setHomePageData(data.homeData); // API'nin döndürdüğü yapı önemli

      // 4. Yeni eklenen slider'ın sliderId’sini bul
      const currentSlide = data.homeData.heroSlides[index];
      if (!currentSlide || !currentSlide.slider_id) {
        alert('Slider ID bulunamadı, resim yüklenemiyor.');
        return;
      }

      // 5. Artık resmi yükle
      await handleImageUpload(file, currentSlide.slider_id);
    } catch (err) {
      console.error('Resim yükleme sırasında hata:', err);
