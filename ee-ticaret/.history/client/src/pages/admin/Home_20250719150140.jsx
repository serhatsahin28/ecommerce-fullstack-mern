...
const AdminHome = () => {
 ...
 const handleImageUpload = async (event) => {
    ...
    const { type, index } = currentImageField; // Artık sliderId'ye buradan ihtiyacımız yok

    // Sadece 'heroSlides' tipindeki yüklemeler için bu mantığı kullan
    if (type !== 'heroSlides' || index === null) {
        setStatusMessage({ show: true, message: 'Geçersiz yükleme bağlamı.', type: 'danger' });
        return;
    }
    
    // GÜNCELLEME: Yüklenecek slaytın tüm verisini state'ten al
    const currentSlideData = homePageData.heroSlides[index];

    const formData = new FormData();
    formData.append('image', file);
    // GÜNCELLEME: Slaytın tüm verisini JSON string olarak ekle
    formData.append('slideData', JSON.stringify(currentSlideData));

    try {
      setStatusMessage({ show: true, message: 'Resim yükleniyor...', type: 'info', dismissible: false });
      ...
      if (response.ok) {
        // GÜNCELLEME: Backend'den gelen tüm güncel 'home' verisiyle state'i yenile
        setHomePageData(result.updatedHome); 
        setStatusMessage({ show: true, message: 'Resim başarıyla yüklendi ve slayt güncellendi!', type: 'success' });
        setShowImageModal(false); // Modalı kapat
      } else {
        // Hata mesajını backend'den geldiği gibi göster
