...
const AdminHome = () => {
  ...
  const handleImageUpload = async (file, sliderId) => {
  const formData = new FormData();
  formData.append('image', file);
  formData.append('sliderId', sliderId);
  formData.append('data', JSON.stringify(homePageData)); // Güncel veri burada
  ...
  try {
    ...
    if (!response.ok) throw new Error(result.message || 'Yükleme başarısız.');

    // Başarılı yükleme sonrası feedback verebilirsin
    console.log('Resim başarıyla yüklendi');
  } catch (err) {
    console.error('handleImageUpload HATA:', err);
    throw err;
  }
};
