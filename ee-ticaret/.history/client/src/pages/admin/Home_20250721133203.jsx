...
const AdminHome = () => {
  ...
  const handleSave = async () => {
    ...
    // Hero slides için slider_id'leri kontrol et
    const cleanedData = {
      ...homePageData,
      heroSlides: homePageData.heroSlides.map(slide => {
        if (!slide.slider_id) {
          return { ...slide, slider_id: uuidv4() };
        }
        return slide;
      })
    };
  ...
  const homePageId = homePageData._id;

  // Sadece heroSlides tipi ve geçerli index için işlem yap
  if (type === 'heroSlides' && index !== null) {
    const currentSlide = homePageData.heroSlides[index];
    const sliderId = currentSlide?.slider_id;

    // 1. DURUM: Slider henüz kaydedilmemiş (slider_id yok)
    if (!sliderId) {
      ...
      setStatusMessage({
        show: true,
        message: 'Önce slider kaydedilmeli! "Tüm Değişiklikleri Kaydet" butonuna tıklayın.',
        type: 'warning'
      });
      return;
    }

    // 2. DURUM: Slider kaydedilmiş (slider_id var)
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/admin/home/upload/${homePageId}`, {
        method: 'POST',
        body: formData
      });
      ...
      if (response.ok) {
        // Backend'den gelen resim yolunu kullan
        ...
        throw new Error(result.message || 'Resim yükleme hatası');
      }
    } catch (err) {
