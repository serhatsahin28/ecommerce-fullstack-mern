...
const AdminHome = () => {
  ...
  // HeroSlides validation kontrolü
  const validatedData = { ...homePageData };
  if (validatedData.heroSlides) {
    validatedData.heroSlides = validatedData.heroSlides.map(slide => ({
      ...slide,
      image: slide.image || '/images/default-slide.jpg' // Varsayılan resim
    }));
  }
  ...
  //     body: JSON.stringify(validatedData) // Düzeltilmiş data gönder
  //   });
...
const addSlideWithPlaceholder = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    // Eğer heroSlides için resim yükleme işlemi yapılıyorsa, slider_id'yi otomatik bul
    let sliderId = null;
    if (type === 'heroSlides' && index !== null) {
      ...
      if (!sliderId) {
        setStatusMessage({
          show: true,
          message: 'Slider ID bulunamadı. Lütfen sayfayı yenileyin ve tekrar deneyin.',
          type: 'danger'
        });
        return;
      }
    ...
    // Sadece heroSlides için sliderId gönder
    if (sliderId) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatusMessage({
          show: true,
          message: 'Resim başarıyla yüklendi! Kaydetmek için "Resmi Kaydet" butonuna tıklayın.',
          type: 'success'
        });
      } else {
        setStatusMessage({
  ...
  const saveImage = async () => {
    ...
    // Resim kaydedildikten sonra tüm verileri kaydet
    await handleSave();

    setStatusMessage({
      show: true,
      message: 'Resim hero slider\'a başarıyla kaydedildi!',
      type: 'success'
    });
  };
                ...
                <ListGroup.Item key={index} className="p-3">
                  <Row>
                    <Col md={10}>
                      ...
                      {/* Slider ID gösterimi (sadece debug için) */}
                      {/* {slide.slider_id && (
                        <small className="text-muted">Slider ID: {slide.slider_id}</small>
                      )} */}
                    </Col>
