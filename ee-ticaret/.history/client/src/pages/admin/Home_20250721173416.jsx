...
import { FaPlus, FaTrash, FaSave, FaSearch, FaTimes, FaImage, FaUpload, FaPencilAlt } from 'react-icons/fa';
...
const AdminHome = () => {
  ...
  // Hangi dilde düzenleme yapıldığını tutar
  ...
  // Slider ekleme/düzenleme modalı
  const [showSlideModal, setShowSlideModal] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(null); // Düzenlenen veya yeni eklenen slaytı tutacak
  const [isEditingSlide, setIsEditingSlide] = useState(false); // Modal'ın "ekleme" mi "düzenleme" mi modunda olduğunu belirtir
  ...
  const handleSlideChange = (field, value, isMultiLang = false, lang) => {
    setCurrentSlide(prev => {
      ...
      if (isMultiLang) {
        return {
          ...prev,
          [field]: { ...prev[field], [targetLang]: value }
        };
      }
    ...
    setHomePageData(prev => {
      const updatedCategories = prev.categories.map(cat => {
        if (cat.category_key === categoryKey) {
          return { ...cat, products: (cat.products || []).filter(p => p.product_id !== productId) };
        }
        return cat;
      });
    ...
    <Container fluid className="p-3 p-md-4">
      ...
      <Row>
        <Col xl={8} className="mb-4">
          ...
          {/* === 1. & 2. İSTEK İÇİN GÜNCELLENEN BÖLÜM === */}
          <Card className="mb-4">
            ...
            <ListGroup variant="flush">
              {homePageData.heroSlides?.map((slide, index) => (
                <ListGroup.Item key={slide._id || slide.slider_id} className="p-3">
                  <Row className="align-items-center">
                    <Col xs={2} md={1}>
                      {slide.image && <img src={slide.image.startsWith('/') ? `http://localhost:5000${slide.image}` : slide.image} alt="Slide" className="img-thumbnail" style={{ width: '50px', height: '50px', objectFit: 'cover' }} />}
                    </Col>
                    <Col>
                      <h6 className="mb-1">{slide.title?.[currentLang] || slide.title?.tr || 'Başlıksız Slayt'}</h6>
                      <p className="text-muted small mb-0">{slide.subtitle?.[currentLang] || slide.subtitle?.tr || 'Alt başlık yok'}</p>
                    </Col>
              ...
              {(!homePageData.heroSlides || homePageData.heroSlides.length === 0) && (
                <ListGroup.Item className="text-center text-muted">Henüz slayt eklenmemiş.</ListGroup.Item>
              )}
            </ListGroup>
          </Card>
        ...
        <Col xl={4}>
          {/* === 3. İSTEK İÇİN GÜNCELLENEN BÖLÜM === */}
          <Card className="mb-4">
      ...
      <Modal show={showSlideModal} onHide={() => setShowSlideModal(false)} size="lg" backdrop="static">
            ...
            <Form>
              <Row>
                <Col md={4}>
                  ...
                  {currentSlide.image && <img src={currentSlide.image.startsWith('/') ? `http://localhost:5000${currentSlide.image}` : currentSlide.image} alt="Slayt Önizleme" className="img-fluid rounded mb-2" />}
                ...
                <Col md={8}>
                  <Tabs defaultActiveKey="tr" id="slide-lang-tabs" className="mb-3">
                    <Tab eventKey="tr" title="Türkçe (TR)">
                      ...
                      <Form.Group className="mb-3"><Form.Label>Buton Yazısı (Opsiyonel)</Form.Label><Form.Control value={currentSlide.cta?.tr || ''} onChange={e => handleSlideChange('cta', e.target.value, true, 'tr')} /></Form.Group>
                    </Tab>
                    <Tab eventKey="en" title="İngilizce (EN)">
                      ...
                      <Form.Group className="mb-3"><Form.Label>Button Text (Optional)</Form.Label><Form.Control value={currentSlide.cta?.en || ''} onChange={e => handleSlideChange('cta', e.target.value, true, 'en')} /></Form.Group>
                    </Tab>
                  </Tabs>
                </Col>
              </Row>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSaveSlide}><FaSave /> {isEditingSlide ? 'Değişiklikleri Kaydet' : 'Slaytı Ekle'}</Button>
        </Modal.Footer>
      </Modal>
