...
const deleteItem = async (req, res) => {
  ...
  try {
    ...
    if (!homeDoc.heroSliders || !Array.isArray(homeDoc.heroSliders)) {
      return res.status(400).json({ message: 'Hero sliders verisi geçersiz.' });
    }
    console.log(homeDoc.heroSliders);

    if (indexNum < 0 || indexNum >= homeDoc.heroSliders.length) {
      return res.status(400).json({ message: 'Geçersiz slider indeksi.' });
    }

    const sliderItem = homeDoc.heroSliders[indexNum];
    console.log(sliderItem);
    // slider_id genelde ObjectId, string karşılaştırması için toString() kullan
    if (!sliderItem || sliderItem.slider_id.toString() !== itemId) {
      return res.status(404).json({ message: 'Belirtilen slider bulunamadı.' });
    }

    // Elemanı çıkar
    homeDoc.heroSliders.splice(indexNum, 1);
