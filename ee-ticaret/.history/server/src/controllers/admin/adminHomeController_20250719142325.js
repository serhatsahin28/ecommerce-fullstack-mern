...
const deleteItem = async (req, res) => {
  ...
  try {
    // Tekil home dokümanını al
    const homeQuery = await home.findOne(); // Eğer sadece bir tane varsa
    ...
    // Eşleşen item bulunuyor mu kontrol et
    const sliderItem = homeQuery.heroSliders[index];
    if (!sliderItem || sliderItem.slider_id !== itemId) {
      return res.status(404).json({ message: 'Belirtilen slider bulunamadı.' });
    }

    // Eşleşeni kaldır
    homeQuery.heroSliders.splice(index, 1);

    // Kaydet
    await homeQuery.save();
