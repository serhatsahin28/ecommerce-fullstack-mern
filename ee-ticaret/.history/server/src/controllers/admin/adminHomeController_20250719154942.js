...
const uploadHomeImage = async (req, res) => {
  try {
    console.log("uploadHomeImage fonk. içerisinde:::");
    const file = req.file;
    const homePageId = req.params.id;
...
// Gerekli alanlar eksikse boş stringle doldur
const fields = ['title', 'subtitle', 'cta', 'cta_link'];
fields.forEach(field => {
  if (!targetSlide[field]) {
    targetSlide[field] = { tr: '', en: '' };
  } else {
    targetSlide[field].tr = targetSlide[field].tr || '';
    targetSlide[field].en = targetSlide[field].en || '';
  }
});

// Sadece image güncelle
