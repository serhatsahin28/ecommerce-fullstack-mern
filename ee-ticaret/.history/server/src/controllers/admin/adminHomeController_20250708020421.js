...
const uploadHomeImage = async (req, res) => {
  try {
    ...
    if (!homePageId || slideIndex === undefined) {
      return res.status(400).json({ message: 'Geçersiz veri veya slideIndex eksik.' });
    }

    // Yol: örn. /images/abc.jpg
    const imagePath = '/images/' + file.filename;

    // İlgili heroSlides elemanını güncelle
    const homePageDoc = await home.findById(homePageId);
    if (!homePageDoc) {
      return res.status(404).json({ message: 'Ana sayfa verisi bulunamadı.' });
    }

    if (!homePageDoc.heroSlides || !homePageDoc.heroSlides[slideIndex]) {
      return res.status(400).json({ message: 'Geçersiz slideIndex.' });
    }

    homePageDoc.heroSlides[slideIndex].image = imagePath;
    await homePageDoc.save();

    res.status(200).json({
      message: `Slide ${slideIndex} resmi başarıyla yüklendi!`,
      imagePath: imagePath
    });
