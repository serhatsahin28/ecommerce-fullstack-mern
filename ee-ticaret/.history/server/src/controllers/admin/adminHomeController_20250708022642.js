...
if (file) {
      // Örneğin resim url'sini updatedData.heroSlides[0].image olarak set etmek gerekebilir
      // Burada, hangi heroSlide'ın güncelleneceğine göre mantık yazılmalı
      // Örnek:
      if (updatedData.heroSlides) {
        // Eğer heroSlides string ise JSON parse edin:
        if (typeof updatedData.heroSlides === 'string') {
          updatedData.heroSlides = JSON.parse(updatedData.heroSlides);
        }
        // İlk sliderın resmini güncelle
        updatedData.heroSlides[0].image = `/uploads/${file.filename}`;
      }
      // Veya başka bir alan güncellenmeli ise benzer şekilde yapabilirsiniz
    }
...
const uploadHomeImage = async (req, res) => {
  try {
    ...
    await home.findByIdAndUpdate(homePageId, imagePath);
